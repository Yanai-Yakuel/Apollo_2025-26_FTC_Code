package comp1_code_folder.firstinspires.ftc.teamcode.autonomous;

import comp1_code_folder.firstinspires.ftc.teamcode.drive.DriveConstants;
import comp1_code_folder.firstinspires.ftc.teamcode.drive.PoseStorage;
import comp1_code_folder.firstinspires.ftc.teamcode.drive.SampleMecanumDrive;
...
@Autonomous(name = "autonomous_FAR_RED", group = "drive")
public class autonomous_far_red extends LinearOpMode {
    ...
    public static double SHOOT_TARGET_RPM = 3050;
    private final double SHOOT_TARGET_VEL = (SHOOT_TARGET_RPM * 28.0) / 60.0;
    public static double B_OPEN = 0.556, B_CLOSE = 0.501;
    public static double hood_open = 0.42;

    private ElapsedTime shootTimer = new ElapsedTime();

    // Start Pose
    Pose2d startPose = new Pose2d(-64, -15, Math.toRadians(0));
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStarted() && !isStopRequested()) {
            telemetry.addData("Ready for RED", selectedMember);
            updateTelemetryPose();
        }
        ...
        drive.followTrajectory(drive.trajectoryBuilder(drive.getPoseEstimate())
                .lineToLinearHeading(new Pose2d(-61.02844, -18.175, Math.toRadians(-25)))
                .build());
        ...
        // Intake entry
        drive.followTrajectory(drive.trajectoryBuilder(drive.getPoseEstimate())
                .lineToLinearHeading(new Pose2d(-32, -30.93, Math.toRadians(-90)))
                .build());
        ...
        drive.followTrajectory(drive.trajectoryBuilder(drive.getPoseEstimate())
                .lineToLinearHeading(new Pose2d(-34, -63.45, Math.toRadians(-90)),
        ...
        // Shot 2
        drive.followTrajectory(drive.trajectoryBuilder(drive.getPoseEstimate())
                .lineToLinearHeading(new Pose2d(-70, -25, Math.toRadians(-20)))
                .build());
        shoot();

        drive.followTrajectory(drive.trajectoryBuilder(drive.getPoseEstimate())
                .lineToLinearHeading(new Pose2d(-36.7224, -30.93, Math.toRadians(-90)))
                .build());
    ...
    private void stopRobot() {
        ...
        telemetry.addLine("Autonomous Complete - Red Side");
        updateTelemetryPose();
    }
