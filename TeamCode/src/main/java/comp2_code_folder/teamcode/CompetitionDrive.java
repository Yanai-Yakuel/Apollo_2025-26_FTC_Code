...
@Config
@TeleOp(name = "Competition_2026_BotPose_FINAL", group = "drive")
public class CompetitionDrive extends LinearOpMode {
    ...
    // Align PID
    public static double HOLD_KP = 0.06;
    public static double HOLD_KI = 0.003;
    public static double HOLD_KD = 0.12;
    ...
    private void executePD(Pose2d currentPose, Pose2d vel, Pose2d target) {
        ...
        if (Math.abs(xErr) < 0.5 && Math.abs(yErr) < 0.5 && Math.abs(hErr) < Math.toRadians(2)) {
        ...
        double xCmd = xErr * HOLD_KP + xIntegral * HOLD_KI - vel.getX() * HOLD_KD;
        double yCmd = yErr * HOLD_KP + yIntegral * HOLD_KI - vel.getY() * HOLD_KD;

        if (Math.abs(xErr) > 0.5)
            xCmd += Math.signum(xErr) * HOLD_MIN_PUSH;

        if (Math.abs(yErr) > 0.5)
            yCmd += Math.signum(yErr) * HOLD_MIN_PUSH;
        ...
        drive.setWeightedDrivePower(new Pose2d(rotX, rotY, hErr * HOLD_KH));
    }
